...
{
    /// <summary>
    /// B series commands API methods.
    /// </summary>
    public class BSeriesController : ApiControllerBase
    {
        ...
        /// Constructor
        /// </summary>
        public BSeriesController(IResponseHelper helper,
        ...
        /// <summary>
        /// Generates an operation code to open the lock using the inputs LOCK ID, USER ID, TOUCH KEY ID, TIME, DATE, TIME BLOCK and LOCK STATUS.
        /// </summary>
        /// <param name="request">OpenLockBRequest</param>
        /// <returns>OperationCodeResponse</returns>
        [Route("OPEN_LOCK_B")]
        public OperationCodeResponse OpenLockB(OpenLockBRequest request)
        {
            ...
            try
            {
                int opCode = this.operationCodeService.GenerateOpenLockBCode(request.LockId, request.UserId, request.TouchKeyId,
        ...
        /// <summary>
        /// Generates an operation code to open the lock using the inputs LOCK ID and USER ID.
        /// </summary>
        /// <param name="request">OpenLockNow1BRequest</param>
        /// <returns>OperationCodeResponse</returns>
        [Route("OPEN_LOCK_NOW_1_B")]
        public OperationCodeResponse OpenLockNow1B(OpenLockNow1BRequest request)
        {
            if (!ModelState.IsValid)
                return this.helper.CreateResponse<OperationCodeResponse>(this.resourceProvider.ResourceString("RequiredFields.Error"), ResponseStatus.Error);
            ...
            try
            {
                int opCode = this.operationCodeService.GenerateOpenLockNow1B(request.LockId, request.UserId);
